...
abstract: "This study investigates factors influencing river water flow in the U.S. Rocky Mountains, with a focus on climate, human activity, and river network characteristics. Using a data set of multiple rivers in the region, two models were applied to address the risk of multicollinearity: Partial Component Regression (PCR) and Lasso Regression. Lasso Regression was ultimately selected for its interpretability and superior predictive performance, explaining 73.93% of the variance in river flow and an out-of-sample RMSE of 0.5118. Results indicate that \emph{Precipitation Seasonality} and \emph{Global Stream Order} significantly impact river water flow. These findings contribute to understanding water management in the Rocky Mountain region."
---
...
This study investigates factors influencing river water flow in the U.S. Rocky Mountains, with a focus on climate, human activity, and river network characteristics. Using a data set of multiple rivers in the region, two models were applied to address the risk of multicollinearity: Partial Component Regression (PCR) and Lasso Regression. Lasso Regression was ultimately selected for its interpretability and superior predictive performance, explaining 73.93% of the variance in river flow and an out-of-sample RMSE of 0.5118. Results indicate that \emph{Precipitation Seasonality} and \emph{Global Stream Order} significantly impact river water flow. These findings contribute to understanding water management in the Rocky Mountain region.
...
```{r fig.width=10, fig.cap = "Scatter plot of spatial displacement of each observation of recorded river flow", echo=F, map}
...
Table \ref{tab:lasso-coefficients} lists and describes the most significant climate, river network, and human factors that impact overall river flow. Of the factors that are most significant are \emph{Precipitation Seasonality}, \emph{Mean Somewhat Excessive Drainage Class}, and \emph{Global Stream Order}.
...
We visually summarize the most significant effects[^4] in Figure 2. For an exact fit, $\hat{Y}=Y$, and hence, for a given factor, he closer an observation is to the equilibrium line, we say the more \emph{influence} that factor had in predicting the water flow metric of that observation. Under this pretext, we acknowledge the large variance in the \emph{Mean Somewhat Excessive Drainage Class}. Table \ref{tab:lasso-coefficients} also records the \emph{inclusion frequency}[^5]. This is a metric of robustness to variation in random sampling. Hence, a larger inclusion frequency indicates a stronger dependency with water flow. We use inclusion frequency in part to assess how well these factors explain overall flow. We point out here that \emph{Precipitation Seasonality} has the highest inclusion frequency.
